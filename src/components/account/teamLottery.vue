        ...
        <div v-if="showCard=='game' " class="tdcp-serach-card" @click="showCard = false">
            ...
            <div class="lottery-play-select">
                <ul class="lottery-play-left">
                    <li v-for="(item,index) in types" v-if="item.isTradition == true && item.show==true "
                ...
                <div class="lottery-play-right">
                    <ul>
                        <li v-if="item.isTradition == true && item.openTradition == true" v-for="item in lotteryList" @click.stop="selectLottery(false,item)" :class="item.name == gameTypeName?'active':''">
                            {{item.name}}
                        </li>
        ...
        methods: {
                    ...
                    for(let i =0;i<this.types.length;i++){
                        ...
                        if(this.types[i].name != 'all'){
                            for(let j =0;j<item.length;j++){
                                if(item[j].isTradition && item[j].openTradition ){
                                    flag = true;
                                    break;
                                }
                            }
            ...
            loadMore(search){
               ...
               // var searchTime=dateUtil.getSearchDateLimit(500);
                ...
                recordSearchApi.getTeamLotteryRecord(params,  (res) =>{
                    if(res.code==200){
                         var data=res.data;
