<template>
    <div  class="kg-lottery-main">
        <dl class="kg-betting-farm" v-if="kgLottery.playTypes!=null  && type.isBall0708  && titleView[type.id]!=null" v-for="type in kgLottery.playTypes">
            <dt>
                <h3 :class="{'active':titleView[type.id/1].arr.length>0}">{{titleView[type.id].ballName}}</h3>
                <p>{{titleView[type.id].desc}}</p>
            </dt>
            <dd v-for="(betType,index) in  type.betTypes" @click="toBetChoice(betType,betType.id,type.id)" :class="{'active':betType.active}">
