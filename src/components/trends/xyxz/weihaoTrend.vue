    ...
    export default {
        ...
        data () {
            return {
                ...
                drawInterval:null,
                cacheMap:{},
                columnNums:[['10-20-30'],['01-11-21-31'],['02-12-22-32'],
                    ['03-13-23-33'],['04-14-24-34'],['05-15-25-35'],['6-16-26-36'],
                    ['07-17-27-37'],['08-18-28-38'],['09-19-29']],
                dataArray:[]
            }
        },
        methods: {
            fillHead(){
                ...
                var columns = xyxzUtil.getWeiNumbers();
                this.secondColumn = columns.secondColumn;
                this.dataArray = this.getColumn(this.secondColumn);
            ...
            getBitMissingNums(){
                ...
                for(var i in _this.issueList) {
                    ...
                    var num  = xyxzUtil.getListLastNumber(nums);
            ...
            getMissing: function (nums) {
                var index =this.selectType;
                var fullNums=this.dataArray[index];
                try {
                        var cnt = true;
                        for(var j=0; j < nums.length; j++){
                            if(arrayUtil.indexOf(fullNums,nums[j])!=-1){
                                this.cacheMap[0] = 0;
                                cnt = false;
                            }
                        }
            ...
            createdNums(nums){
                ...
                var fullNums = _this.dataArray[index];
                    var cnt = true;
                    for(var j=0; j < nums.length; j++){
                        if(arrayUtil.indexOf(fullNums,nums[j])!=-1){
                            xyxzUtil.everyNumSetColor(nums[j],missingList);
                            cnt = false;
                        }
                    }
                    if(cnt){
                        missingList.push({num:this.cacheMap[0]});
                    }
